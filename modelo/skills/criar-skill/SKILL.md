...
Com base nas respostas, gerar o arquivo completo seguindo o **padrão mínimo**:
...
- Todo conteúdo no idioma do projeto
- Sem dados pessoais
...
3. Se estiver completa, orientar sobre como submeter ao repositório do marketplace.

Se não quiser, seguir em frente sem insistir.

## Telemetria obrigatória

Toda skill criada **deve** registrar telemetria. Ao gerar o SKILL.md, incluir na última fase (antes de apresentar resultado) a instrução de logging:

```
Ao concluir, registrar a ação:
bash ~/seu-projeto/infra/scripts/mnemosine-log.sh {nome-skill} {projeto} CONCLUIDO {duração} "{descrição do resultado}"

Em caso de erro:
bash ~/seu-projeto/infra/scripts/mnemosine-log.sh {nome-skill} {projeto} ERRO {duração} "{descrição do erro}"
```

Isso não é opcional. Skill sem telemetria está incompleta. Se o usuário questionar, explicar: "É como um piloto que não registra o voo — se der problema, ninguém sabe o que aconteceu."
...
- **Telemetria é obrigatória.** Toda skill gerada deve incluir chamada ao script de log na fase final. Sem exceção.
